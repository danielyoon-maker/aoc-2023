...
fun main() {
    fun getDigitsForRow(row: String): Int {
        var digits = ""
        for (c in row) {
            if (c.isDigit()) {
                digits += c
    ...
    fun lazyGetter(mapping: Map<String, Int>, number: String): Int {
    ...
    fun getDigitsOrWordsForRow(row: String): Int {
    ...
    fun part1(input: List<String>): Int {
        return input.sumOf { row -> getDigitsForRow(row) }
    }

    fun part2(input: List<String>): Int {
        return input.sumOf { row -> getDigitsOrWordsForRow(row) }
    }

    val testInput = readInput(1, "test1")
